		...
		<table class="table">
			<tr>
				...
				<td><select name="ma_category" required>
						...
						<option value="부자재">부자재</option></td>
			</tr>
			...
			<tr>
				<td>원산지</td>
				<td><input type="text" name="ma_origin" value="${ma.ma_origin}"
					readOnly></td>
			</tr>

			<tr>
				<td>유통기한</td>
				<td><input type="date" name="ma_expiryDate"
					value="${ma.ma_expiryDate}" readOnly></td>
			</tr>

			<tr>
				<td>재고수량</td>
				<td><input type="number" name="ma_stockQuantity"
					value="${ma.ma_stockQuantity}" id="ma_stockQuantity"></td>
			</tr>
			...
			<tr>
				<td>단위</td>
				<td><input type="text" name="ma_unit" value="${ma.ma_unit}"
					readOnly></td>
			</tr>

			<tr>
				<td>포장규격</td>
				<td><input type="text" name="ma_specifications"
					value="${ma.ma_specifications}" readOnly></td>
			</tr>

			<tr>
				<td>가용재고량</td>
				<td><input type="number" name="ma_availableStock"
					value="${ma.ma_availableStock}" readOnly></td>
			</tr>

			<tr>
				<td>기본재고량</td>
				<td><input type="number" name="ma_basicStock"
					value="${ma.ma_basicStock}" readOnly></td>
			</tr>

			<tr>
				<td>단가액</td>
				<td><input type="number" name="ma_price" value="${ma.ma_price}"
					id="ma_price"></td>
			</tr>

			<tr>
				<td>총금액</td>
				<td><input type="number" name="ma_stockValue"
					value="${ma.ma_stockValue}" readOnly id="ma_stockValue"></td>
			</tr>
			...
			<tr>
				...
				<td><select name="ma_storage" required>
						...
						<option value="물류창고">물류창고</option></td>
			</tr>
			...
			<tr>
				<td>첨부파일번호</td>
				<td><select name="attachment_no" required>
			...
			<td><input type="hidden" name="ma_id" id="ma_id"
				value="${ma.ma_id}"></td>
		</table>
...
<script>
	...
	function calStockValue() {
		var quantity = parseFloat(document
				.querySelector('input[name="ma_stockQuantity"]').value) || 0; // 재고 수량 값 없으면 0
		var price = parseFloat(document.querySelector('input[name="ma_price"]').value) || 0; // 가격 값 없으면 0
		document.querySelector('input[name="ma_stockValue"]').value = quantity
				* price; // 총액
	}

	// 재고 수량 입력 필드 입력 -> 총액 갱신
	document.querySelector('input[name="ma_stockQuantity"]').addEventListener(
			'input', calStockValue);
	// 가격 입력 필드 입력 -> 총액 갱신
	document.querySelector('input[name="ma_price"]').addEventListener('input',
			calStockValue);
