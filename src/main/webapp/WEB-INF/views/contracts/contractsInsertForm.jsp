...
<script>
	...
	function calculate(){
	    ...
	    // 결과를 화면에 표시 (소수점 없이)
	    document.getElementById("contracts_price").value = Math.floor(contracts_price); // 소수점 제거
	}//end
	...
	function checkdate() {
	    ...
	    if (contracts_deliveryDate < today) {
	        alert("제조소요일이후의 날짜여야합니다."); // 오늘 이전 날짜 선택 시 경고
	        form.contracts_deliveryDate.value = ""; // 입력값 초기화
	        form.contracts_deliveryDate.focus(); // 계약일 필드에 포커스
	        return;
	    }
	...
	function btnsumbit(){
		...
		var contracts_deliveryDate = document.getElementById("contracts_deliveryDate").value;
...
<div class="content-body">
	<div>
		...
		<form action="contractsInsert" method="post" name="contractsInsertForm" id="contractsInsertForm">
			<div>
				<h4>물품 상세</h4>
				<label>계약물품코드</label> <input name="item_no" id="item_no" type="number" placeholder="물품선택" onclick="openitemModal()"><br>
				<label>계약물품분류</label> <input name="item_category" id="item_category" type="text" placeholder="물품선택" onclick="openitemModal()"><br>
				<label>계약물품명</label> <input name="item_name" id="item_name" type="text" placeholder="물품선택" onclick="openitemModal()"><br>
				<label>계약물품단가</label> <input name="item_price" id="item_price" type="number" placeholder="물품선택" onclick="openitemModal()" oninput="calculate()"><br>
				<label>물품제조소요일(일)</label> <input name="bom_schedule" id="bom_schedule" type="number" placeholder="물품선택" onclick="openitemModal()">
			</div>
			<div>
				<label>계약수량</label> <input name="contracts_quantity" id="contracts_quantity" type="number" oninput="calculate()">
			</div>
			<div>
				<label>계약가격</label> <input name="contracts_price" id="contracts_price" type="number" onkeyup="calculate()" readonly>
			</div>
			<div>
				<label>계약납기일</label> <input name="contracts_deliveryDate" id="contracts_deliveryDate" type="date" onchange="checkdate()">
			</div>
			<div>
				<label>계약상세</label>
				<textarea id="contracts_details" name="contracts_details" placeholder="계약상세내용"></textarea>
			</div>
			<div>
				<h4>계약협력회사 상세정보</h4>
			...
			<div>
				<button type="button" onclick="btnsumbit()">계약등록</button>
