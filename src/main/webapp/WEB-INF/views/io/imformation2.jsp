...
/* 테이블 전체 스타일 */
 ...
 if(selectedData.length > 0){
     ...
     if(itemcheck === true ){
  		  ...
  		  if (itemcheck) {
            // 데이터를 URL 파라미터로 변환
           		 var queryString = selectedData.map(function(item) {
               		 return 'obtain_no=' + encodeURIComponent(item.obtain_no) + '&ma_id=' + encodeURIComponent(item.ma_id);
           			 }).join('&');
	...
	io_btn.onclick = function(){
		...
		console.log(document.querySelectorAll('input[name="iocheck"]:checked').value);
